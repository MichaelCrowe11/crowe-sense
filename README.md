...
> **Status:** Firmware implemented and tested — 25 hardware-free unit tests pass (`ruff` clean). CI pipeline runs lint + tests + per-part STL renders. Enclosure is parametric OpenSCAD with a published BOM. Hardware bring-up validation is the remaining frontier.
